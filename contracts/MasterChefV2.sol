...
contract MasterChefV2 is Ownable, ReentrancyGuard {
  ...
  function add(string memory _name, uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
  ...
  function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
    add("", _allocPoint, _lpToken, _depositFeeBP, _withUpdate);
  }

  // Update the given pool's EGG allocation point and deposit fee. Can only be called by the owner.
  function set(uint256 _pid, string memory _name, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
  ...
  function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
    set(_pid, poolInfo[_pid].name, _allocPoint, _depositFeeBP, _withUpdate);
  }
  ...
  function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
    ...
    if (_amount > 0) {
      ...
      if (pool.depositFeeBP > 0) {
        ...
        uint256 devBon = depositFee.mul(2500).div(10000);
        pool.lpToken.safeTransfer(devaddr, devBon);
        pool.lpToken.safeTransfer(feeAddress, depositFee.sub(devBon));
